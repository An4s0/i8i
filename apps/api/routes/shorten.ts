...
router.post("/shorten", async (req, res) => {
    try {
        ...
        const ipAddress = forwardedIP ? forwardedIP.split(",")[0] : req.socket.remoteAddress || "Unknown";
        if (!ipAddress) {
            res.status(400).json({
                success: false,
                message: config.messages.error.invalid.ip
            } as APIResponse);
            return;
        }
        ...
        if (!urlPattern.test(originalUrl)) {
            res.status(400).json({
                success: false,
                message: config.messages.error.invalid.url
            } as APIResponse);
            return;
        }
        if (days != null && (days < 1 || days > 365)) {
            res.status(400).json({
                success: false,
                message: config.messages.error.invalid.days
            } as APIResponse);
            return;
        }
        if (password != '' && (password.length < 6 || password.length > 20)) {
            res.status(400).json({
                success: false,
                message: config.messages.error.invalid.password
            } as APIResponse);
            return;
        }
        ...
        res.status(200).json({
            success: true,
            message: config.messages.success.shorten,
            data: {
                shortUrl: `${config.urls.baseUrl}/c/${shortCode}`,
                originalUrl,
...
router.get("/shorten", async (req, res) => {
    try {
        ...
        if (!shortCode) {
            res.status(400).json({
                success: false,
                message: config.messages.error.invalid.shortCode
            } as APIResponse);
            return;
        }
       ...
       await new analyticsModel({
        ...
        res.status(200).json({
            success: true,
            message: config.messages.success.retrieved,
            data: {
                shortUrl: `${config.urls.baseUrl}/c/${findShortUrl.shortCode}`,
