...
export default function AnalyticsResults({ data, shortCode }: { data: [AnalyticsFormat], shortCode: string }) {
    const options = [
    ...
    const visitsData: Record<string, { label: string, Visits: number }[]> = {
        "24hours": Array.from({ length: 24 }, (_, i) => {
            ...
            const count = visits.filter(v => v.date >= start && v.date < end).length;
    ...
    const referrersData = groupByLabel("referrer");
        ...
        <main className="max-w-7xl m-auto space-y-2 p-3">
            ...
            <div className="flex justify-between space-x-1">
                <div className="flex items-center space-x-1">
                    ...
                    <Link href={config.urls.baseUrl + "/c/" + shortCode} target="_blank">
                        <p className="text-primary font-semibold">
                            {config.urls.baseUrl + "/c/" + shortCode}
                        </p>
                    </Link>
                </div>
            ...
            <div className="w-full max-w-7xl border border-subtle/30 rounded-md mt-5 h-[400px]">
                <Visits data={visitsData[selected.value]} />
            </div>
